...
<hr>
	...
	<img src="<%=request.getContextPath() %>/resources/img/강아지4.gif" width="50">
	<a href="input_comunityboard" class="btn btn-primary btn-lg" role="button">글 작성하기</a> 
	<img src="<%=request.getContextPath() %>/resources/img/강아지4.gif" width="50">
	</h3> 
</hr>
<hr>
<form class="text-center" action="list_comunityboard" method="get">
		제목으로 검색<input type="text" name="search_word"><input type="submit" value="검색">	
	</form><br>
 <table class="table table-hover"> 
  ...
  <!--  리스트를 가져옴.. foreach -->
  	<tbody>
	<c:forEach var="comunityboardvo" items="${cList}" varStatus="status">  <!--  리스트나 배열을 제어하는 명령어 -->
 			<tr>
 				<td style="vertical-align: middle" class="text-center">${status.count}</td>
 				<td style="vertical-align: middle" class="text-center">${comunityboardvo.num}</td>
 				<td style="vertical-align: middle" class="text-center"><a href="comunityboard_detail?viewno=${comunityboardvo.num}"><img src="<%=request.getContextPath() %>\resources${comunityboardvo.img_thumb }" width="50" /></td>
 				<td style="vertical-align: middle" class="text-center">${comunityboardvo.animal_kind}</td>
 				<td style="vertical-align: middle" class="text-center"><a href="comunityboard_detail?viewno=${comunityboardvo.num}">${comunityboardvo.title }</td>
 				<td style="vertical-align: middle" class="text-center">${comunityboardvo.content}</td>
 				<td style="vertical-align: middle" class="text-center">${comunityboardvo.writer}</td>
 				<td style="vertical-align: middle" class="text-center"><fmt:formatDate value="${comunityboardvo.indate}" pattern="yyyy-MM-dd"/></td>
 				<td style="vertical-align: middle" class="text-center">${comunityboardvo.cnt}</td>
 	...
 	<div>	
 		<ul class="text-center">
 			<c:if test="${pageVO.prev }">
 				<a href="list_comunityboard?page=${pageVO.startPage -1 }&search_word=${pageVO.search_word }">[이전페이지그룹]</a>
 			</c:if>
 			<!-- page 그룹 정보출력 변수명 startPage   endPage -->
 			<c:forEach begin="${pageVO.startPage }" end="${pageVO.endPage }" var="idx">
 				<a href="list_comunityboard?page=${idx }&search_word=${pageVO.search_word }">${idx }</a>
 			</c:forEach>
 			<c:if test="${pageVO.next }">
 				<a href="list_comunityboard?page=${pageVO.endPage +1 }&search_word=${pageVO.search_word }">[다음페이지그룹]</a>
 			</c:if>
 		</ul>
